...
def build_and_solve(scheduling_horizon_weeks, solve_time_seconds=30):
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        # =========================================================
        # 18. EXPORT SUBMISSION CSV FILES
        # =========================================================
        # The hackathon expects three files for each scenario:
        #   SCHEDULE_ACCESS.csv
        #   SCHEDULE_OCCUPANCY.csv
        #   RESULTS.csv
        #
        # Files are written to output_A / output_B / output_C.
        ...
        for _, export_activity in activity_data.iterrows():
            activity_id = export_activity["activity_id"]
            max_nights = int(
                export_activity["number_of_maximum_access_per_week"]
            )

            selected = []
            for week in weeks:
                for night in range(1, max_nights + 1):
                    if solver.Value(access[activity_id, week, night]) == 1:
                        selected.append(
                            (
                                week,
                                night,
            ...
            for access_seq, (week, night, eclo_value) in enumerate(
                selected, start=1
            ):
                access_rows.append(
                    {
        ...
        for _, export_activity in activity_data.iterrows():
            ...
            for week in weeks:
                if solver.Value(occupied[activity_id, week]) != 1:
                    continue
                ...
                for location_id in activity_locations[activity_id]:
                    ...
                    if selected_group is None:
                        raise RuntimeError(
                            f"No co-share group found for {activity_id}, "
            ...
            results_rows.append(
                {
                    ...
                    "simulated_completion_date": (
                        simulated_completion_date.strftime("%Y-%m-%d")
                    ),
        ...
        print(
            "Rows:",
            len(schedule_access_df), "access |",
            len(schedule_occupancy_df), "occupancy |",
            len(results_df), "results"
        )
