    ...
    for j in range(i + 1, len(activity_ids)):
        ...
        for week in weeks:
            ...
            for location_id in common_locations:
                capacity = location_capacity[location_id]

                for group in range(1, capacity + 1):
...
for _, activity in activity_data.iterrows():
    ...
    # For the first scheduled week, use horizon_weeks + 1 for unscheduled
    ...
    for week in weeks:
        ...
        model.Add(
            first_candidate == horizon_weeks + 1
        ).OnlyEnforceIf(
            week_is_used.Not()
        )
